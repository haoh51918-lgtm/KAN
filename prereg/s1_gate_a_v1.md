# S1 Gate A Preregistration v1

> Frozen before any scientific E1–E5 result is produced. `KAN_Alpha_PR.md` is the sole method authority. This document supersedes `prereg/s1_gate_a_v0.md` for the scientific run; v0 remains an immutable historical record.

## Supersession record

No result from seeds `1729`, `2718`, or `31415` was trained, selected, promoted, or test-opened under v0. Only reduced one-step fresh-seed connectivity smokes were observed, so no scientific Gate metric exists. A code audit corrected non-duplication to cover the initial dictionary's complete input/output four-affine semantics. Under that correct audit, the v0 mechanism's best initial primitive was an affine `Tanh`, with Pearson `0.999053` and NRMSE `0.043509` on the governance grid, below the frozen `0.05` duplicate threshold. Thus v0 violated its own deliberately-out-of-dictionary design premise. V1 changes only the mechanism constants before any scientific result; all arms, seeds, budgets, thresholds, controls, and evidence boundaries remain unchanged.
...
The v1 clean response is deliberately more asymmetric than v0 and remains absent from the initial primitive dictionary:

\[
g(x)=
\begin{cases}
-0.6(1-\exp(2.5x)), & x<0,\\
 1.8(1-\exp(-0.25x)), & x\ge 0.
\end{cases}
\]
...
Only after eligibility is established, fit the three prospectively declared low-complexity monotone families to the frozen residual samples using train/validation shapes. Non-duplication fits each initial primitive with its full input scale, input bias, output scale, and output bias under the bounded audit declared in `configs/model_specs/s1_gate_a_promotion_v0.json`. The test clean target cannot select the family. The promoted primitive receives a new versioned ID and must pass domain, boundary, stability, non-duplication, low-complexity approximation, and executable reconstruction checks.

Hardening closes every free spline path, retains the selected analytical primitives and their input affines, inserts the governed primitive on the frozen source/window, and refits only continuous output affine scales/intercept on train plus validation. The E4-soft-to-HARD fidelity metrics audit the compression loss. If no governed primitive passes, the result remains semi-symbolic and Gate A fails.

## Metrics

For every seed and arm report: clean and noisy test NRMSE; shape NRMSE on the fixed 801-point mechanism grid; exact source/window and centered contribution mass; soft-to-hard Pearson/NRMSE/max error; residual raw energy and ratio; executable node count/depth/free constants/description length; trainable parameters, steps, candidate evaluations, estimated FLOPs, wall time, and peak memory. Aggregate by median and full seed range. Add a within-seed circular date-block bootstrap percentile interval, but do not claim a population p-value from three seeds.

## Negative controls

For E4, rerun one matched budget per seed with both training-objective and validation-selection labels permuted by whole date blocks of length 20. A healthy pipeline must not promote a primitive on any null run. The exact `Return(Close,5)` column is also removed in a separate diagnostic without reconstruction or future access.

## Gate A thresholds

Gate A passes only when all conditions hold:

1. **Capacity sanity:** E1 or C6 achieves median clean test NRMSE at most `0.15`; otherwise the run is technically inconclusive.
...
5. **Executable promotion:** the hardened model has median clean test NRMSE at most `0.15`, soft–hard correlation at least `0.98` in every successful seed, and median soft–hard NRMSE at most `0.10`.
6. **Interpretability advantage:** E4's hardened result is Pareto-undominated across clean NRMSE, hardening fidelity, and description length; neither E3/E5 nor E2 may be no worse on all axes and strictly better on one.
7. **Null safety:** zero of three permuted-label runs promotes a primitive.

Gate A fails scientifically when capacity sanity passes but any condition 2–7 fails after valid runs and allowed implementation recovery. It is invalidated and rerun when parity, causal masking, split isolation, budget accounting, implementation snapshot, or test-once rules fail.
