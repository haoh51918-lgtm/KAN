# KAN Alpha 因子挖掘框架：原始研究构想与方案草案

> **当前文档角色（2026-07-17）**：这是项目的原始 idea draft、假设目录和设计备选，不是当前实施手册、agent 契约或唯一权威。文中的“必须”“不可动摇”“锁定”“常设方案”等措辞只记录当时的提案意图，不能授权当前实验、数据 opening、方案切换或外部发送。当前行为遵循 `AGENTS.md`，当前科学状态遵循 `docs/research/MIRAGE_KAN_LIVING_MANUAL.md`；冻结预注册只约束各自已经命名的实验。

> 面向 CSI300 日频横截面数据的可解释 Alpha 发现研究  
> 文档性质：对“KAN 框架优化与挑战”完整会话的结构化重写  
> 推荐主方案：**MIRAGE-KAN——图控制的符号—残差 KAN 矿机协同进化**  
> 适用读者：研究者、算法工程师、量化研究员，以及需要理解项目起源的 AI agent  
...
> 修订（2026-07-17）：新增第 25 节——研究者首要动机重申：KAN 可解释性主线不可动摇；可解释性的操作性定义与验收；分解可辨识性五条修复路线；S1b 升格为主线必做；S2 矿机池 KAN 化。第 25 节与旧章节冲突时以第 25 节为准，裁决表见 25.9。Gate A v1 封存负结果不受本修订影响。
...
- 一份历史性的后续执行建议；当前 agent 不应将其视为契约。
...
25. 当时的核心动机与 KAN 可解释性主线（2026-07-17 历史修订）
...
应对：把 KAN 价值限定为连续孔洞优化、函数形状发现和符号忠实度；若实证不支持，应调整论文主张。（2026-07-17 修订）按第 25 节：本风险的应对只允许收缩主张表述与证据边界，不得触发 KAN 核心地位降级、不得引入黑盒矿机；方案更换权仅属 principal。
...
> 本节整合外部评审意见。**第一目标不变：完整 MIRAGE-KAN 闭环（Plan A）。**每个关键 Gate 预先准备一条可发表的降级路径，使任何实验结果都有出路；（2026-07-17 修订）原自动绑定降为建议映射，任何方案切换需 principal 书面批准（见 21.2 注与第 25 节），与核心动机冲突的 Plan C 路径已废止。降级不是放弃，而是把已完成的工作重组为更小但完整的证据链。

### 21.1 决策总则

1. 主方案优先：资源分配、实现顺序、实验设计都按 Plan A 展开；
2. 降级**建议**可由 Gate 结果或里程碑倒排触发，不由主观感觉触发；（2026-07-17 修订）任何降低 KAN 核心地位或引入黑盒矿机的方案切换，必须经 principal 书面批准后方可执行——Gate 结果本身只自动收缩主张边界，不自动更换方案；
3. 任何时点只有一个"当前核心主张"，其余内容降为组件、消融或未来工作；
4. 切换 Plan 时写决策日志：触发证据、切换日期、被搁置的模块清单。

### 21.2 Gate 与路径绑定表

> （2026-07-17 修订）本表由自动绑定改为**建议映射**：任何方案切换必须由 principal 书面批准后方可生效；未获批准前，常设主线始终是 MIRAGE-KAN（第 25 节常设方案声明）。原 Plan C 行已废止。

| 触发条件 | 路径 | 论文形态 |
|---|---|---|
| 全部 Gate 通过、进度可达 M5 | **Plan A（常设第一目标）** | MIRAGE-KAN 完整闭环 |
| Gate A 通过、Gate B 失败或进度不达 M4 | Plan B | Symbolic-Residual KAN 单独成文（KAN 仍为核心） |
| Gate A 失败 | ~~Plan C~~ **已废止（2026-07-17）** | 改走第 25 节：S1b 可辨识性修复主线；KAN 核心地位不变 |
| Gate A、B 均失败且 S1b 亦失败 | Plan D（仅作诚实发表出口） | 合成机制恢复基准 + 系统性负向结果分析；系统仍为 MIRAGE-KAN，不引入黑盒矿机 |
...
### 21.4 Plan C（已废止，2026-07-17）：图控闭环为主角、KAN 去中心化

> **废止声明**：本路径与 principal 的核心动机——KAN 高可解释性为卖点、KAN 为不可动摇的挖掘核心、不引入 MLP 等黑盒矿机——直接冲突，经 2026-07-17 principal directive（第 25 节）彻底放弃。下段摘要仅作历史记录与决策审计保留；**任何后续 agent 不得再尝试、部分恢复或变相实现此路径**，除非 principal 以书面决策日志主动更换方案。
>
> 历史摘要：Plan C 曾设想在 Gate A 失败后将 KAN 降为矿机实例之一，引入 MLP/GP 矿机同台运行，以图控闭环为论文主角。2026-07-16 Gate A 失败后曾短暂激活（`governance/decisions/2026-07-16_gate_a_plan_c.md`，该决议同日被部分取代，见其 supersession note），2026-07-17 被第 25 节取代。其中与动机不冲突的证据纪律——真实数据回测优先、图必要性三层基线（14.2 节）、完整对照与统计纪律——已并入第 25 节与 S1b/S2 设计继续执行。
...
> 本节不是现行 agent 契约。凡涉及当前阶段、强制步骤、方案绑定或权限的内容，均以 `AGENTS.md` 和 Living Manual 为准。
...
若 Symbolic-Residual KAN 在表达力—解释性 Pareto 上不优于纯符号搜索或 MLP，则暂停上层图系统，启动 S1b 修复并收缩主张边界；KAN 的核心角色不因此改变（第 25 节）。

**（2026-07-17 修订）Gate A 失败 → 不再触发任何 KAN 降级路径。** 原"启动 21.4 节 Plan C"指令已随 Plan C 一并废止（见 21.4 废止声明与第 25 节）。现行指令：Gate A 失败 → 按 25.6/25.7 启动 S1b 可辨识性修复主线（新预注册、新种子、机制 battery），KAN 核心地位与 MIRAGE-KAN 常设主线不变；论文主张按 25.4 收缩为"可解释性交付 + 性能非劣效"。15.8 节第 9 项（字典外原语恢复）的前置要求保留，由 S1b 机制 battery 承接。方案更换权仅属 principal。
...
> 本节记录 2026-07-17 当时对本提案的修订，只用于解释后续历史决策。它不再拥有跨文档优先级，也不约束当前路线。已封存的 Gate A v1 `scientific_fail` 证据与治理记录仍按其物理证据保留（见 25.5）。

### 25.1 首要动机声明（当时的固定主张）

本研究计划的**最核心动机是以 KAN 的高可解释性为卖点**：

1. **可解释性优先于性能。** 即使挖掘性能仅与 baseline 接近（非劣效，见 25.4），只要因子具备机制语义明确、人类可读可分析的性质，研究即达成首要目标。性能超越 baseline 是加强证据，不是成立前提。
2. **因子必须"知其所以然"。** 交付的每一个因子都必须能回答"这个因子是怎么来的、它在计算什么、为什么可能有效"，而不是黑盒搜索碰运气碰出来的高分表达式。
3. **该版提案把 KAN 固定为因子挖掘核心。** 挖掘系统的生产路径（miner/production path）以 KAN 系矿机为核心构建；MLP 等黑盒结构不进入矿机池。当前项目是否继续这一选择由证据和最新 principal 指令决定。
4. 本节**彻底废止**第 21.4 节 Plan C 路径（其正文已改写为带禁令的历史摘要）：KAN 回归主角。图模块的锁定状态及其解锁条件（S2 证明因子库价值、S3 证明冗余问题）不变。
5. **该版提案的常设方案**是 MIRAGE-KAN。这个历史声明不阻止当前项目根据新证据改变系统核心；任何实际变更仍需遵循当前 `AGENTS.md` 的权限边界。

### 25.2 可解释性的操作性定义与验收

"高可解释性"必须可检验，否则会退化为宣传语。本项目中，一个因子被允许称为**可解释**，当且仅当同时满足三个条件：

| 条件 | 定义 | 载体 |
|---|---|---|
| **可读** | 最终因子是 typed DSL 闭式公式（§4.5 等级 1—2），受 MDL/AST 深度约束；样条残差若保留必须以显式命名原语形式出现 | 因子库 manifest 中的 canonical AST |
| **可分析** | 机制卡（§16）完整：每条边/每个原语的形状图、变量贡献质量、窗口语义、软—硬忠实度（NRMSE/Pearson）、单调与饱和区间、一句话机制陈述、反事实响应方向（给定输入扰动 → 因子响应符号） | 每因子一张机制卡，随库发布 |
| **可追溯** | 从因子可回溯"它是咋来的"的完整链条：产生它的 KAN run ID、种子、门控选择序列、硬化收据、被拒绝的备选结构、全部尝试计数 | 谱系账本 + 硬化收据 + attempts ledger |

**验收测试（发布门槛）**：§15.9 盲评协议升格为强制门槛。盲评者（不了解本项目的量化研究员）仅凭机制卡须能 (a) 复述该因子的机制陈述；(b) 正确预测给定输入变化下因子的响应方向。未达预注册一致性门槛的因子，只能以"计算透明"名义发布，**不得**进入"可解释"主张——这是对"代数黑话"风险（§20.5）的硬约束。

**表述边界**：对外表述遵守 §2.2 的三层解释边界。本项目承诺的是**计算透明性 + 行为可解释性**（公式、形状、响应方向可由人类完整复述与审计）；"经济机制解释"逐因子按机制卡证据分级陈述，不作一揽子宣称。"物理意义"一词在论文中应替换为"机制语义明确"，避免 §2.3 警示的过度主张。

### 25.3 黑盒结构的角色边界

- **禁止**：MLP、无约束深网络及任何不可硬化为 DSL 公式的结构，不得作为矿机、不得产出入库因子、不得出现在因子库的任何生产环节。
- **保留（且必须保留）**：容量匹配的黑盒对照（如 Gate A 的 C6 型 MLP）继续存在于**评价协议**中。原因：没有黑盒对照，"KAN 在可解释的同时性能不吃亏"这一核心主张不可证伪，无法通过评审。对照是证伪仪器，不是矿机；其输出永不入库。
- 纯符号搜索（typed GP/SR，E5 型）同理从"主要可执行路径"降为**对照基线臂**：它是"KAN 的可微搜索是否优于离散搜索"（RQ1）的证伪对象，其产出可用于方法比较，但主线因子库以 KAN 系矿机产出为主体。

### 25.4 成功判据重排序

主主张（论文级）：

1. **可解释性交付**：正式因子库 100% 因子通过 25.2 验收（等级 1—2 + 机制卡 + 盲评）；
2. **性能非劣效**：在冻结 Quanta 协议下，KAN 因子库对选定 baseline 达到非劣效。非劣效边界 δ 在 S2 预注册中冻结；建议默认：净 Information Ratio 的块 bootstrap 95% 置信下界 ≥ baseline − 0.1·|baseline IR|（TOST 风格，呼应附录 F 第 12 条），同时 RankIC、最大回撤、换手成本无实质性恶化（阈值同样预注册冻结）。

次主张：若同时统计上显著优于 baseline，作为加强证据报告。

诚实条款：**不得**用可解释性叙事掩盖统计上显著更差的性能；若显著更差，必须如实报告"可解释性—性能"权衡并把它作为结果本身。多重检验、完整尝试记账、置换对照等 §11.2/§15.10 纪律不因判据重排而放松。

### 25.5 与 Gate A v1 封存负结果的关系

- Gate A v1 `scientific_fail` 保持封存：不重开、不重跑、不重新解释。已消耗的种子 1729/2718/31415 永久退役。
- 该负结果证伪的是**一种具体架构假设**——"联合训练下加性分解可辨识、样条残差会自动对齐未知机制"。它没有证伪 KAN 可解释挖矿主线，也没有证伪字典外原语发现本身。
- 本节全部内容为方法与目标层面的前瞻修订，制定于 S1b/S2 任何新结果观察之前，符合"先注册后实验"纪律。

### 25.6 分解可辨识性的五条修复路线

Gate A 诊断（三种子一致的非单调补偿曲线；v0/v1 量化出的仿射闭包陷阱）指向以下修复菜单。R1/R2/R4 为 S1b 主力臂，R3 为消融臂，R5 为治理修订、与前四条正交。

| # | 路线 | 机制 | 针对的已观测失败模式 | 文献锚点 |
|---|---|---|---|---|
| R1 | 形状约束残差 | 残差样条空间直接限制为"可提升锥"（单调、零点锚定、饱和），B 样条系数经 cumsum/softplus 重参数化保证；非可提升形状不可表示，训练结局二选一：找到可提升形状，或残差萎缩为零 | 非单调找补曲线被构造性排除 | MonoKAN (arXiv:2409.11078)；SCAM/SCOP-splines (Pya & Wood 2015)；单调归纳偏置 (arXiv:2606.17886) |
| R2 | Sequential / hard-first 分解 | 先仅用字典拟合并冻结解析部分，再让样条单独拟合残差；可交替回拟合至收敛，最终联合微调 + 忠实度审计 | 联合训练的分工不可辨识（"抬沙发问题"） | Robinson (1988) 双残差；DML Neyman 正交 (Chernozhukov et al. 2018)；kanpiler 冻结注入 (KAN 2.0, arXiv:2408.10205)。Plan C 决议已预授权此选项 |
| R3 | 正交惩罚 | 联合训练中惩罚样条输出对当前解析张成空间（含四仿射闭包）在数据测度上的投影 | 同 R2 的可微化版本；张成空间随门控漂移，工程风险高 | 半参数正交化传统；仅作消融臂 |
| R4 | 形状级离散分解 | KAN 只负责把总形状学准（Gate A 已证明其为全场最佳），分解移到净形状网格上做离散搜索："字典表达式 + 一个自由参数 family"的组合按 MDL 选优；硬化管线升级为端到端感知的算子选择 | 梯度下降会滑入任意方便分解；离散枚举显式比较多个候选分解 | AI Feynman (arXiv:1905.11481; 2006.10782)；GSR/GMP 端到端硬化 (arXiv:2603.15250) |
| R5 | 提升判据改革 | 新原语准入从"形状非重复"改为/并列"跨任务复用压缩"：候选原语加入字典后跨 K 个挖掘任务重跑轻量纯符号挖掘，要求样条依赖、描述长度、搜索成本系统性下降（MDL over corpus） | 仿射闭包陷阱——v0 机制被 affine-Tanh 以 0.0435 复制、v1 真值机制仅 2 倍余量逃过 0.05 查重，说明"形状距离"判据渐近自我封死；真实数据无形状真值，复用压缩是唯一可操作的新颖性定义 | DreamCoder (arXiv:2006.08381)；Stitch (arXiv:2211.16605)；本文档 §4.11 判据 4 的形式化 |

### 25.7 S1b 升格为主线必做

S1b 从 Plan C 决议中的"可选非阻塞分支"升格为**主线必做实验**（仍不阻塞 S2 的真实数据链路）：

- **三主力臂**：E4-mono（R1）、E4-seq（R2）、E4-shapeSR（R4），共享同一批新机制与新种子；R3 作为消融臂视预算加入。
- **机制 battery**：实现 §15.8 第 1—9 类全部机制，报告**方法 × 机制类的恢复率矩阵**，替代单机制过/不过判定。单调饱和类之外的机制（驼峰、状态切换等）用于如实测量 R1 的召回边界，防止形状约束的盲区被隐藏。
- **半真实层**：真实 OHLCV 面板 spike-in 注入已知机制，位于合成与实盘之间的中间证据层。
- **治理**：新预注册、新科学种子、独立一次性 test opening、完整置换对照；提升判据按 R5 预注册；不复用 v1 任何已消耗种子；沿用 Gate A 的封存/收据/索引machinery。
- **成本证据**：Gate A 正式全矩阵（6 臂 × 3 种子 + 控制 + 提升治理）实测单卡约 21 分钟（artifact 时间戳 22:41:45→23:02:17）；三臂 × battery 为小时级，不构成进度风险。
- **结果绑定**：S1b 通过（预注册的恢复率门槛）→ Plan B（Symbolic-Residual KAN 单独成文，§21.3）复活，且叙事采用"诚实报告 v1 失败 → 展示可辨识修复"的完整弧线；S1b 失败 → 双重负结果 + 机制恢复基准按 §15.8/Plan D 独立成文，KAN 仍按 25.8 作为纯符号矿机继续主线。

### 25.8 S2 的 KAN 化改造

1. **主力矿机（立即可投产，不依赖 S1b 结果）**：纯符号 KAN 矿机（E3 架构：边上只有解析原语门控，训练中离散化，直接产出等级 1 公式）。其可解释性天然满足 25.2 的"可读"条件。
2. **第二矿机（S1b 后加入）**：S1b 胜出臂的半符号→硬化产线，产出等级 1—2 因子。
3. **对照臂**：typed GP/SR 基线（方法比较用）；容量匹配黑盒对照（性能非劣效检验用）。两者产出均不入库。
4. **发布纪律**：正式因子库只接受 §4.5 等级 1—2 因子；等级 3—4 仅作内部诊断，不得进入回测主张；每个入库因子附机制卡与完整来源链；`kan_mined` 字段必须如实标注产出矿机类型。
5. **既有 S2a v1 预注册的处置**：本次 proposal 修订有意使 `prereg/s2_plan_c_vertical_v1.lock.json` 的 proposal 哈希失配（系统将按设计 fail-closed 拒绝旧协议执行）。S2a 在预执行审计 BLOCK 状态下未消耗任何测试资源，故按本节重新预注册为 v2（矿机池 KAN 化、判据按 25.4）并重建全部锁后方可执行。旧 v1 预注册与锁保留为历史记录，不删除。

### 25.9 章节冲突裁决表

| 旧条款 | 处置 |
|---|---|
| §21.4 Plan C"KAN 去中心化、图为主角"路径 | **彻底废止**（正文改为带禁令的历史摘要，未经 principal 书面决策不得再尝试）；图模块锁定状态与解锁条件不变 |
| §21.2 自动绑定表、§21.1 规则 2、§22.4"Gate A 失败→Plan C"指令、§20.1 应对 | **就地改写**：自动降级绑定改为 principal-only 批准；Plan C 触发指令替换为 S1b 主线指令 |
| Plan C 决议第 2 条"typed GP/symbolic 为主要可执行路径" | **修订**：KAN 符号矿机为主力，GP/SR 降为对照基线臂 |
| Plan C 决议"保留 MLP…作为辅助矿机" | **收紧**：MLP 移出矿机池，仅保留为评价协议中的容量匹配对照 |
| Plan C 决议"可选非阻塞 S1b" | **升格**：主线必做（仍不阻塞 S2 真实数据链路） |
| `prereg/s2_plan_c_vertical_v1.md` 及其锁 | **前瞻性作废**（未消耗任何测试资源）；需按 25.8 重新预注册为 v2；旧文件保留为历史 |
| Gate A v1 封存结果、种子退役、全部治理事故记录 | **不变** |
| §14 基线族（含 C6 型对照、Boost-Sequential、置换控制） | **不变**（证伪仪器，非矿机） |
| §11.2/§15.10 统计纪律、§16 机制卡、§15.9 盲评 | **不变并强化**（盲评升格为发布门槛，见 25.2） |

本节当时触发的行动已成为历史：决策日志、S1b 预注册和后续 S2a 协议均应从各自文件读取，不应由本段再次触发。
