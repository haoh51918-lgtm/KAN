"""Establish the frozen v4 runtime before lock, mining, or development imports."""
...
TRACKING_RELATIVE = Path("evaluations/runtime/s2a_v4_tracking")
...
def _configure_torch() -> dict[str, object]:
    ...
    if state != expected:
        raise RuntimeError("v4 deterministic Torch state did not hold")
    return state
...
def _tracking_receipt(
    ...
    return {
        ...
        "schema_version": "s2a_v4_tracking_preimport_receipt_v1",
...
def main() -> int:
    ...
    if Path(sys.prefix).resolve() != expected_prefix:
        raise RuntimeError("runtime launcher must use runtime/s2a_v4_eval/.venv")
    ...
    if args.mode == "tracking-receipt":
        ...
        staged = pending.stage_decision(workspace / ".s2a_v4_decision.staging")
