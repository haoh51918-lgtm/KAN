"""Establish the frozen v5 runtime before lock, mining, or development imports."""
...
TRACKING_RELATIVE = Path("evaluations/runtime/s2a_v5_tracking")
...
def _configure_torch() -> dict[str, object]:
    ...
    if state != expected:
        raise RuntimeError("v5 deterministic Torch state did not hold")
    return state
...
def _tracking_receipt(
    ...
    return {
        ...
        "schema_version": "s2a_v5_tracking_preimport_receipt_v1",
...
def main() -> int:
    ...
    if args.mode == "tracking-receipt":
        ...
        staged = pending.stage_decision(workspace / ".s2a_v5_decision.staging")
